...
public class PlayerFile {
	...
	public static boolean fileExists(UUID path) {
		File file = new File("plugins/BookStats/UserData/" + path + ".yml");
		if (!(file.exists())) {
			return false;
		}
		return true;
	...
	public void save(boolean use) {
		if (map.containsKey(file.getName()) == true && Bukkit.getScheduler().isCurrentlyRunning(map.get(file.getName()))) {
			Bukkit.getScheduler().cancelTask(Integer.parseInt(map.get(file.getName()).toString()));
			map.remove(file.getName());
		}
		task = Main.inst().getServer().getScheduler().runTaskAsynchronously(Main.inst(), new Runnable() {
			@Override
			public void run() {
				try {
					map.put(file.getName(), task.getTaskId());
					yaml.save(file);
					map.remove(file.getName());
				} catch (Exception e) {
